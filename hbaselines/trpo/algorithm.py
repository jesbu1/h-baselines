...
from stable_baselines.common import explained_variance, zipsame, dataset, \
    fmt_row, colorize, ActorCriticRLModel, SetVerbosity, TensorboardWriter
from stable_baselines import logger
from stable_baselines.common.mpi_adam import MpiAdam
from stable_baselines.common.cg import conjugate_gradient
from stable_baselines.common.policies import ActorCriticPolicy
from stable_baselines.a2c.utils import total_episode_reward_logger
from stable_baselines.trpo_mpi.utils import traj_segment_generator, \
    add_vtarg_and_adv, flatten_lists
...
class TRPO(ActorCriticRLModel):
    ...
    def learn(self,
        ...
        with SetVerbosity(self.verbose), \
                TensorboardWriter(self.graph, self.tensorboard_log,
            ...
            with self.sess.as_default():
                ...
                while True:
                    if callback is not None:
                        # Only stop training if return value is False, not when
                        # it is None. This is for backwards compatibility with
                        # callbacks that have no return statement.
                    ...
                    for k in range(self.g_step):
                        ...
                        observation, action = seg["observations"], \
                            seg["actions"]
                        ...
                        with self.timed("computegrad"):
                            ...
                            # run loss backprop with summary, and save the
                        ...
                        if np.allclose(grad, 0):
                            ...
                            for _ in range(10):
                                ...
                                if not np.isfinite(mean_losses).all():
                                    logger.log("Got non-finite value of "
                                               "losses -- bad!")
                                elif kl_loss > self.max_kl * 1.5:
                                    logger.log("violated KL constraint. "
                                               "shrinking step.")
                                elif improve < 0:
                                    logger.log("surrogate didn't improve. "
                                               "shrinking step.")
                                else:
                            ...
                            if self.nworkers > 1 and iters_so_far % 20 == 0:
                                ...
                                assert all(np.allclose(ps, paramsums[0])
                                           for ps in paramsums[1:])

                        with self.timed("vf"):
                            for _ in range(self.vf_iters):
                                # NOTE: for recurrent policies, use
                                # shuffle=False?
                    ...
                    if self.using_gail:
                        ...
                        # list of tuples, each of which gives the loss for a
                        # minibatch
                        d_losses = []
                        ...
                        for ob_batch, ac_batch in dataset.iterbatches(
                                ...
                                shuffle=True):
                            ob_expert, ac_expert = \
                                self.expert_dataset.get_next_batch()
                            # update running mean/std for reward_giver
                            if self.reward_giver.normalize:
                                self.reward_giver.obs_rms.update(
                                    np.concatenate((ob_batch, ob_expert), 0))

                            # Reshape actions if needed when using discrete
                            # actions
                            if isinstance(self.action_space, Discrete):
